...
class Rectangle:
    def __init__(self, x, y, size):
        ...
        self.x, self.y = x, y
        self.size = size
        self.vertices = np.array([(i[0] * self.size + self.x, i[1] * self.size + self.y) for i in ((-1, 1), (1, 1), (1, -1), (-1, -1))])
        ...
        self.edges = [
            ...
            shapes.Line(*self.vertices[3], *self.vertices[0], batch=batch),
        ]
...
class Fractal:
    ...
    def __init__(self, rectangle):
        ...
        self.point = np.array((float(self.r.x), float(self.r.y)))
...
r = Rectangle(height//2, width//2, 450)
f = Fractal(r)
